...
const cards = ref<Card[]>([
  ...
  },
  {
    ...
    a: ["A change (to the Constitution)", "An addition (to the Constitution)"],
  },
  {
  ...
  <div class="mx-auto max-w-xl p-4 select-none">
    ...
    <div class="relative h-170 [perspective:1000px]">
      ...
      >
        <!-- Front -->
        <div
          class="absolute inset-0 rounded-2xl border p-6 grid place-items-center text-center text-2xl font-medium bg-white [backface-visibility:hidden] text-black"
        >
          <span>{{ current.q }}</span>
        </div>
        ...
        >
          <div class="space-y-3 text-2xl">
            ...
            <template v-else>
              <div class="text-2xl font-semibold">{{ current.a }}</div>
            </template>

            <div
              v-if="current.alsoAcceptable?.length"
